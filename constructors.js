...
//Name and email will be strings, age will be a number and savedPosts will be an array of objects. These objects will each have 3 properties: 
//id (a number), title (a string), and rating (a number between 1 and 5). These objects are the posts that the user will have saved to their account.
...
//It should take in three parameters: id (a number), title (a string) and rating (a number). Use these parameters to create a new object 
//and add it to the savedPosts array. Make sure to name the properties the same as described previously (id, title, rating).
...
//one number parameter representing the post id. Use this id to find and remove the matching object in the savedPosts array.

// Code here
User.prototype.removeSavedPost = function(num){
let index = this.savedPosts.findIndex(elem => elem.num === num)
...
let newProperties = this.savedPosts.splice(index, 1, newRating)
this.savedPosts.push(newProperties)
}
